...
const AppController = {
    ...
    try {
        ...
        if (!errors.isEmpty()) {
          ...
          AccountTemplate.service
            .view({ template: template, account_id: account._id })
            .then(async function (document) {
              ...
              for (const key in data.content) {
                if (Object.hasOwnProperty.call(data.content, key)) {
                  document_content_locale = document_content_locale.replace(
                    "{{" + key + "}}",
                    data.content[key]
                  );
                }
              ...
              for (const key in data.subject) {
                if (Object.hasOwnProperty.call(data.subject, key)) {
                  subject_locale = subject_locale.replace(
                    "{{" + key + "}}",
                    data.subject[key]
                  );
                }
              ...
              if (account.email_service === "smtp") {
                const smtp = await CustomSmtp.service.view({
              ...
              const sendMailQueue = new bullQueue(
                "sendMailQueue",
                config_cache.cache.redis_host
              );
  ...
  async accountTemplates(request, response, next) {
    ...
    const templates = await AccountTemplate.service.listWithTemplates({
      account_id: id,
    });
    response.status(200).json({
      type: AppConstants.RESPONSE_SUCCESS,
  ...
  async deliverabilityInsights(request, response, next) {
    const { id } = request.user;
    const Insights = await DeliverabilityInsights.service.list({
      account_id: id,
    });
    return response.status(200).json({
      type: AppConstants.RESPONSE_SUCCESS,
  ...
  async refreshToken(request, response, next) {
    const errors = validationResult(request);
    if (!errors.isEmpty()) {
      response.status(422).json({ type: "error", message: errors.array() });
      return;
    } else {
