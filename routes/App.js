      ...
      return Account.service.findAccountByEmail(value).then(function (email) {
        if (email) {
          return Promise.reject(
            value + "already exists, please choose another"
          );
        }
      });
    }),
...
router.post(
  ...
  [
    ...
    check("variables").not().isEmpty().withMessage("variables is required"),
  ],
...
router.put(
  ...
  [
    ...
    check("subject").not().isEmpty().withMessage("subject is required"),
    check("content").not().isEmpty().withMessage("content is required"),
    check("variables").not().isEmpty().withMessage("variables are required"),
  ],
...
router.post("/email-templates/search", TemplateController.search);
    ...
    check("template").custom(function (value) {
      return Template.service
        ...
        .then(function (template) {
          if (!template) {
            return Promise.reject(
              value + " does not exists, please choose another"
            );
          }
        });
    }),
...
router.get(
  "/me/templates",
  AuthenticationGuard,
  AppController.accountTemplates
);

router.get(
  "/me/templates/:id",
  AuthenticationGuard,
  AppController.showAccountTemplates
);
...
router.delete(
  "/me/templates/:id",
  AuthenticationGuard,
  AppController.deleteAccountTemplates
);

router.get(
  "/me/deliverability-insights",
  AuthenticationGuard,
  AppController.deliverabilityInsights
);
